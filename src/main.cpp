...
int main(int argc, char **argv){
    ...
    for(const auto& pair : network.shapes) {
        const bht::Shape& shape = pair.second; // Korrekt: bht::Shape ist der Typ
    ...
    for(const auto& pair : network.stopTimes) {
        const bht::StopTime& stopTime = pair.second;
        std::cout << "Trip ID: " << stopTime.tripId 
              << ", Arrival Time: " << stopTime.arrivalTime
              << ", Departure Time: " << stopTime.departureTime
